...
This recreates the data and figure for figure SI2
...
def set_cost_mig_BH(cost, mig, B_H):
    ...
    if B_H > 0:
        ...
        model_par_local['D_H'] = sd
    return model_par_local


#run model - paramter scan - end point
def run_model():
    ...
    saveName = data_folder / 'data_FigureSI2.npz'
...
def run_model_single():
    ...
    model_par_D['B_H'] = 0
    model_par_D['D_H'] = model_par['s'] / (1 + model_par['s'])

    model_par_B = model_par.copy()
    model_par_B['B_H'] = model_par['s']
    model_par_B['D_H'] = 0

    modelParList = (model_par_B, model_par_D)
...
#rload or run model - single parameter set - time dynamics
def load_model_single():
    parToIgnore = ('B_H', 'D_H', 'TAU_H', 'maxT')  # need not check these parameters
    loadName = data_folder / 'data_FigureSI2_part2.npz'
    if loadName.is_file():
        # open file and load data
        file = np.load(loadName, allow_pickle=True)
        OutputB = file['OutputB']
        OutputD = file['OutputD']
        InvPerHostB = file['InvPerHostB']
        InvPerHostD = file['InvPerHostD']
        Output = (OutputB, OutputD)
        InvPerHost = (InvPerHostB, InvPerHostD)
    ...
    if rerun:
        # rerun model
        Output, InvPerHost = run_model_single()
...
#load or  model - paramter scan - end point
def load_model():
    # need not check these parameters
    parToIgnore = ('cost', 'mig', 'B_H', 'D_H')
    loadName = data_folder / 'data_FigureSI2.npz'
    if loadName.is_file():
        # open file and load data
...
def plot_heatmap(fig, ax, data1D):
    ...
    fig.colorbar(im, ax=ax, 
                 label="$\\log_{2} \\frac{\\langle f \\rangle_{birth}}{\\langle f \\rangle_{death}}$",
...
def plot_histogram_line(axs, data):
    ...
    sd = model_par['s']/(1+model_par['s'])
    axs.legend(('$s_b$=%.1f' % model_par['s'], '$s_d=%.1f$' % sd), loc='upper left')
    return None
