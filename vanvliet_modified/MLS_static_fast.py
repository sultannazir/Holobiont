...
def init_comm(model_par):
    ...
    return (cAct, dAct, micx, micy, Hgene)
...
# sample model
def sample_model(c, d, binEdges, output, InvestmentPerHost, sample_idx, currT, model_par,micx,micy,Hgene):
    #store microbe dynamics
    output["mic_dyn"][sample_idx] = micx/(micx+micy)
    #store host gene frequency
    output["Hgene_freq"][sample_idx] = Hgene.mean()
    # store time
...
@jit(Tuple((f8[::1], f8[::1], f8[::1], i8))(f8[::1], f8[::1], f8[::1], f8[::1], i8, f8, f8, f8[:, ::1], i8, f8[::1]))
def update_host(CVec, DVec, AgeVec, cumulPropVec, numSubStep, n0, sigma, rndMat, ridx, Hgene):
    # init  vectors that keep track of changes in host
    numGroup = CVec.size
    cNewTemp = np.zeros(numSubStep)
    dNewTemp = np.zeros(numSubStep)
    HgeneTemp = np.zeros(numSubStep)
    numNewBorn = 0
    ...
    Hgene = np.concatenate((Hgene[hasDied == 0], HgeneTemp[0:numNewBorn]))
    AgeVec = np.concatenate((AgeVec[hasDied == 0], np.zeros(numNewBorn)))

    return(CVec, DVec, AgeVec, ridx, Hgene)
...
@jit(void(f8[::1], f8[::1], f8, f8, f8, f8, f8, f8, f8[::1]), nopython=True)
def update_comm(c, d, cost, mu, mig, dt, micx, micy, Hg):
    nGroup = c.size
...
def run_model_fixed_parameters(model_par):
    ...
    # init groups
    CVec, DVec, micx, micy, Hgene = init_comm(model_par)
    ...
    sampleIndex = sample_model(CVec, DVec, binEdges,
                               ...
                               sampleIndex, currT, model_par,micx,micy, Hgene)
    ...
    while currT <= maxT:
        ...
        # update hosts
        CVec, DVec, AgeVec, tiR, Hgene = update_host(
            CVec, DVec, AgeVec, cumulPropVec, numSubStep, n0, sigma, rndMat, tiR, Hgene)

        # update age
        AgeVec += dtBac

        # next update community
        update_comm(CVec, DVec, cost, mu, mig, dtBac, micx, micy, Hgene)

        micx0 = micx
        micy0 = micy
        micn = micx0 + micy0
        micx += dtBac * (micx0 * ((1 - mu) * (1 - cost) - micn) + mu * micy0)
        micy += dtBac * (micy0 * ((1 - mu) - micn) + mu * (1 - cost) * micx0)
...
def single_run_finalstate(MODEL_PAR):
    ...
    dType = np.dtype([
        ...
        ('TAU_H', 'f8'), ('B_H', 'f8'), ('D_H', 'f8'), ('K_H', 'f8'), ('mu', 'f8'), ('mic_dyn', 'f8'), ('Hgene_freq', 'f8')])
    ...
    output_matrix['mic_dyn'] = Output['mic_dyn'][-1]
    output_matrix['Hgene_freq'] = Output['Hgene_freq'][-1]
