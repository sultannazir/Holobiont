...
figureName = 'figure3sigma2.pdf'
dataName = 'data_Figure3sigma2' \
           '.npz'
...
cost_vec = np.logspace(*costRange, nStep)
GH_vec = np.logspace(*GHRange, nStep+3)
B_H_vec = [model_par['s'], 0]  #sb values
#tau_H_vec = 1 / desiredTauV
...
def set_cost_GH_BH(GH, cost, B_H, sigma):
    model_par_local = model_par.copy()
    model_par_local['TAU_H'] = GH
...
def run_model():
    ...
    modelParList = [set_cost_GH_BH(*x)
                    for x in itertools.product(*(GH_vec,cost_vec, B_H_vec, sigma_vec))]
...
def load_model():
    ...
    parToIgnore = ('TAU_H', 'cost', 'B_H', 'sigmaBirth', 'rms_err_treshold')
...
def process_data(statData):
    ...
    dataToStore = (sigma_cat, BH_cat)
    nameToStore = ('sigma_cat', 'BH_cat')
...
def plot_3D(ax, data1D, sigmaIndex):
    ...
    ax.set_xlim(costRange)
    ax.set_ylim(GHRange)
    ax.set_zlim(fRange)
    ax.set_xticks(np.linspace(*costRange, steps[0]))
    ax.set_yticks(np.linspace(*GHRange, steps[1]))
    ax.set_zticks(np.linspace(*fRange, steps[2]))

    # set labels
    ax.set_xlabel('$log_{10} \\gamma$')
    ax.set_ylabel('$log_{10} G_H$')
...
def plot_heatmap(fig, ax, data1D, sigmaIndex):
    ...
    xStep = 0.5
    yStep = 0.5
    xbins = np.linspace(*costRange, int(
        np.ceil((costRange[1]-costRange[0])/xStep))+1)
    ybins = np.linspace(*GHRange, int(
        np.ceil((GHRange[1] - GHRange[0]) / yStep)) + 1)
